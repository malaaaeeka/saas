...
if (data.success) {
  ...
  if (loading) return <p className="text-muted">Loading CA partners...</p>

  return (
    <div className="max-w-7xl">
      <h1 className="text-3xl font-bold mb-2">CA Partners</h1>
      <p className="text-muted mb-8">Manage chartered accountant partners</p>

      {cas.length > 0 ? (
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {cas.map((ca) => (
            <div key={ca.id} className="bg-surface border border-border rounded-lg p-6 hover:border-accent transition">
              <h3 className="text-lg font-bold mb-2">{ca.firmName}</h3>
              <p className="text-muted text-sm mb-4">ICAP: {ca.icapNumber}</p>
              
              <div className="space-y-3 mb-6 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted">Clients:</span>
                  <span className="font-semibold text-link">{ca.clientCount || 0}</span>
                </div>
                <div className="flex justify-between">
              ...
              <button className="w-full bg-btn-dark hover:bg-btn-dark-hover text-btn-dark-text px-4 py-2 rounded text-sm font-medium transition">
                View Details
              </button>
            </div>
          ))}
