...
export default function ProductDetailPage() {
  ...
  // Gated on `product` being loaded, since we filter invoices by product.description
  useEffect(() => {
    const token = localStorage.getItem('token')
    if (!token || !product?.description) return
    fetchInvoices(token, page, pageSize)
    fetchInvoiceCounts(token)
  }, [page, pageSize, typeFilter, statusFilter, search, product?.description])
    ...
    <div className="min-h-screen bg-background text-heading p-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-3xl font-bold mb-2">{productLoading ? 'Loading...' : (product?.description || 'Product')}</h1>
          <p className="text-muted">
  {product ? `HS Code: ${product.hsCode || '—'}${product.uom ? ` · ${product.uom}` : ''}${product.rate ? ` · Rate: PKR ${Number(product.rate).toFixed(2)}` : ''}` : ''}
</p>
          </div>
          ...
          <div className="fixed inset-0 bg-background z-50 overflow-y-auto">
            <div className="max-w-4xl mx-auto px-8 py-12">
              <div className="flex justify-between items-center mb-8">
                <span className="text-xs text-muted uppercase tracking-widest">Search Invoices for {product?.description || 'Product'}</span>
