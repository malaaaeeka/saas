...
import { parseProductRows, ProductRow } from '@/lib/productExcel'
...
export default function BulkUploadProductsPage() {
  ...
  const handleFile = async (e: any) => {
    ...
    try {
      ...
      const parsed = parseProductRows(dataRows, rawHeaders)
      setRows(parsed)
      setSuccess(`Loaded ${parsed.length} product(s) from ${file.name}`)
    } catch {
  ...
  const requestSubmit = () => {
    if (validCount === 0) { setError('No valid products to submit'); return }
    setShowConfirm(true)
  }
    ...
    for (let i = 0; i < chunks.length; i++) {
      try {
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/products/bulk`, {
          ...
          body: JSON.stringify({ products: chunks[i].map(r => r.payload) }),
        })
    ...
    <div className="min-h-screen bg-background text-heading p-8">
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-muted">Upload an Excel file to create multiple products at once</p>
        </div>
          ...
          <>
            <div className="mb-6">
              ...
              <button type="button" onClick={() => fileInputRef.current?.click()}
                ...
                <div className="flex-1 min-w-0">
                  <p className="text-heading font-semibold text-sm">Upload Excel with multiple products</p>
                  <p className="text-muted text-xs mt-0.5">One row per product — use the template for the expected columns</p>
                </div>
            ...
            {rows.length > 0 && (
              <div className="bg-surface rounded-xl p-6 border border-border shadow-sm mb-6">
                <h2 className="text-lg font-semibold mb-2">Preview — {rows.length} Product(s)</h2>
                  ...
                  <table className="w-full text-sm">
                    <thead className="bg-border-light text-left">
                      <tr>
                        ...
                        <th className="p-3 font-medium text-muted">HS Code</th>
                        <th className="p-3 font-medium text-muted">Rate</th>
                    ...
                    <tbody>
                      {rows.map(r => (
                        <tr key={r.rowNumber} className="border-t border-border">
                          ...
                          <td className="p-3">{r.payload.description || '—'}</td>
                          <td className="p-3 font-mono text-xs">{r.payload.hsCode || '—'}</td>
                          <td className="p-3">{r.payload.rate !== null ? `PKR ${r.payload.rate}` : '—'}</td>
        ...
        {results && (
          <div className="bg-surface rounded-xl p-6 border border-border shadow-sm">
            ...
            <button type="button" onClick={() => router.push('/products')}
              className="bg-btn-dark hover:bg-btn-dark-hover text-btn-dark-text font-semibold py-3 px-8 rounded-lg transition mt-6">Go to Products</button>
          </div>
        )}
          ...
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
            <div className="bg-surface rounded-xl p-6 border border-border shadow-lg max-w-md w-full">
              ...
              <p className="text-muted text-sm mb-6">
                {skippedCount > 0 ? `${skippedCount} product(s) will be skipped due to validation errors. Submit the remaining ${validCount}?` : `Submit ${validCount} product(s)?`}
              </p>
